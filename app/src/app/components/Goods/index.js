...
export default function Goods({ currentPage, setCurrentPage }) {
    ...
    const parts = pathname.split("/home/items/");
   ...
   console.log(catId);
    ...
    const getVariantText = (count) => {
        if (count === 1) return "варіант";
        if (count >= 2 && count <= 4) return "варіанти";
        return "варіантів";
    };
    const count = goods.filter((obj) => obj.category === catId).length;
    const variantText = getVariantText(count);
    ...
    //     item.firstLoan.toLowerCase().includes(inputValued.toLowerCase())
    // );
    ...
    return (
        ...
        <div className={styles.wrapper}>
            <h3 className={styles.countCredits}>{count === 0 ? "На жаль, варіантів немає." : `Ми підібрали для вас ${count} можливих ${variantText}.`}</h3>
